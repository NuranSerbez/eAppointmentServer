...
namespace eAppointmentServer.Application.Features.Appoinments.CreateAppointment
{
    ...
    IPatientRepository patientRepository) : IRequestHandler<CreateAppointmentCommand, Result<string>>
    {
        public async Task<Result<string>> Handle(CreateAppointmentCommand request, CancellationToken cancellationToken)
        {
            ...
            Console.WriteLine($"StartDate from request: {request.StartDate}");
            Console.WriteLine($"EndDate from request: {request.EndDate}");
           ...
           .AnyAsync(p => p.DoctorId == request.DoctorId &&
            ...
            {
                return Result<string>.Failure(404, ["Appointment date is not available"]);
            }
